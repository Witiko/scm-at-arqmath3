...
docker run --rm -it -u "$(id -u):$(id -g)" --hostname "$HOSTNAME" --runtime=nvidia -e NVIDIA_VISIBLE_DEVICES="$GPUS" -p "$PORT:$PORT" -v "$PWD"/..:/workdir:rw -w /workdir "$IMAGE_NAME" jupyter-lab --ip 0.0.0.0 --port "$PORT"
